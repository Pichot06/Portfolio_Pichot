---
title: "À propos"
layout: "about"
url: "/about/"
summary: À propos
showtoc: true
tocopen: true
date: 2024-07-09
---

### 👋 Qui suis-je ?

Je m’appelle Alexandre Pichot, diplômé du DUT GEII à l’IUT de Nice, passionné de domotique, robotique et nouvelles technologies. Mon objectif est simple : concevoir des solutions intelligentes pour rendre notre quotidien plus sûr, plus confortable et plus durable.

### 🏫 Mon parcours : IUT GEII à Nice

J’ai intégré l’IUT GEII à Nice, une formation d’excellence qui m’a permis d'acquérir des bases solides en :

- Électronique
- Automatique
- Informatique industrielle
- Électrotechnique

Ce cadre dynamique et exigeant m’a permis de travailler sur de nombreux projets concrets et de découvrir la rigueur du travail en équipe.

### 🤖 Mon moteur au quotidien


Depuis longtemps, j’ai une passion pour le bricolage électronique : que ce soit en réparant des objets ou en bidouillant des projets à base de microcontrôleurs, j’aime comprendre, démonter et améliorer les systèmes qui m’entourent.

La robotique m’attire particulièrement, car elle allie mécanique, électronique et intelligence embarquée. Pour moi, c’est l’expression la plus aboutie de l’ingénierie : créer des systèmes autonomes, capables d’interagir avec leur environnement, représente un défi technique que je trouve à la fois stimulant et passionnant.

Je m’intéresse et je m’exerce régulièrement sur :

- Les microcontrôleurs (Arduino, ESP32, STM32, Mbed)
- La programmation embarquée
- L’intégration de capteurs variés
- Les systèmes domotiques (Home Assistant)

Actuellement, je développe plusieurs projets personnels mêlant objets connectés et robotique autonome, avec pour objectif de concevoir des solutions intelligentes et modulables.

Ce qui me motive, c’est de donner vie à des systèmes capables d’agir de manière autonome et utile dans le monde réel.